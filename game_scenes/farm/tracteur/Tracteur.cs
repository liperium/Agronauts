...
public partial class Tracteur : CharacterBody2D
{
	...
	public override void _Ready()
	{
		...
		timer.WaitTime = random.NextDouble()*5.0;
		streamPlayer2D = GetNode<AudioStreamPlayer2D>("AudioStreamPlayer2D");
		timer.Timeout += () =>
		{
			locked = false;
			streamPlayer2D.Play();
	...
	public void ManualProcess(double delta)
	{
		...
		if (Input.IsActionPressed("tracteur_front") || Input.IsActionPressed("tracteur_back"))
		{
			...
			streamPlayer2D.PitchScale = Mathf.Lerp(streamPlayer2D.PitchScale,1.0f+velocity.Normalized().Length(),0.3f);
			MoveAndCollide(velocity);
		}
		...
		if (Input.IsActionJustReleased("tracteur_front") || Input.IsActionJustReleased("tracteur_back"))
		{
			streamPlayer2D.PitchScale = Mathf.Lerp(streamPlayer2D.PitchScale,1.0f,0.3f);
		}
	}
